...
- **Фронт ещё не ходит в API.** Сервер отвечает на `:4000` (HTTP и сокеты),
  но страницы по-прежнему импортируют `src/mocks` напрямую — axios появится
  в фазе 1.3. Пока это единственное, что разделяет две готовые половины.
