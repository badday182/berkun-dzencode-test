...
- **Транспорт до API есть, вызовов нет.** `src/services/http.ts` —
  сконфигурированный axios-инстанс с нормализацией ошибок, но им пока никто не
  пользуется: страницы по-прежнему импортируют `src/mocks` напрямую.
  Типизированные методы и thunks — следующие пункты фазы 1.3.
